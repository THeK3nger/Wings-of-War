...
float PLANE1_INIT_POS [3] = {718, 288, M_PI};
float PLANE2_INIT_POS [3] = {50, 288, 0};
...
void Field::init() {
  ...
  _plane1 = new Plane(0, 10, PLANE1_INIT_POS[0], PLANE1_INIT_POS[1], PLANE1_INIT_POS[2]);
  _plane1->plane_sprite.SetColor(sf::Color(255, 0, 0));        // PLANE 1 IS RED

  LOGMESSAGE("Initialize Plane 2");
  _plane2 = new Plane(1, 10, PLANE2_INIT_POS[0], PLANE2_INIT_POS[1], PLANE2_INIT_POS[2]);
  ...
  _brain1 = new WoWBrain(_plane1, _theWorld);
  _brain2 = new WoWBrain(_plane2, _theWorld);
  _brain1->search_depth = Game::conf.ai1_search_depth;
  _brain2->search_depth = Game::conf.ai2_search_depth;
...
void Field::reset(){
  ...
  _plane1->setX(PLANE1_INIT_POS[0]); _plane1->setY(PLANE1_INIT_POS[1]); _plane1->setT(PLANE1_INIT_POS[2]);
    _plane2->setX(PLANE2_INIT_POS[0]); _plane2->setY(PLANE2_INIT_POS[1]); _plane2->setT(PLANE2_INIT_POS[2]);
...
void Field::update() {
  ...
  case Field::INIT:
    ...
    this->_internal_state = Field::AI1_SELECT;
    break;

  case Field::AI1_SELECT:
    if(_paused){
      break;
    }
    
    ai1_choices =  _brain1->returnBestCards();
	  
    // change state
    this->_internal_state = Field::AI2_SELECT;
#if DEBUG
    LOGMESSAGE("AI1 has chosen!");
#endif
    break;
  case Field::AI2_SELECT:
    ai2_choices = this->_brain2->returnBestCards();
#if DEBUG
    LOGMESSAGE("AI2 has chosen!");
#endif
  ...
  case Field::APPLY_MOVES:
    ...
    _plane1->move(ai1_choices[moves_counter]);
    _plane2->move(ai2_choices[moves_counter]);
...
#endif
      _plane2->inflictDamage(this->_brain2->expectedDamage());
      _playerLifebar->setLife(this->_brain2->expectedDamage());
    }
    if ((!plane2_out) && _plane2->canShootTo(_plane1)){
      _plane1->inflictDamage(this->_brain1->expectedDamage());
      _enemyLifebar->setLife(this->_brain1->expectedDamage());
#if DEBUG
